...
function TestsRunner (server, appPath, userConfig) {
    this.server = server;
...
TestsRunner.prototype.runSingleTarget = function (target, savePath) {
    ...
    return this.prepareAppToRunTarget(target).then(function() {
        ...
        logger.normal("Collecting logs for the devices.");
        var logPath = me.userConfig.getLogPath()? me.userConfig.getLogPath(): me.appPath;
        var logMins = me.userConfig.getLogMins()? me.userConfig.getLogMins(): util.DEFAULT_LOG_TIME;
        var paramedicLog = new ParamedicLog(target.platformId, me.appPath, logPath);
        ...
        if(me.userConfig && me.userConfig.shouldCleanUpAfterRun()){
            logger.normal("Killing the process.");
            paramedicKill(target.platformId);
        }
        return results;
    })
